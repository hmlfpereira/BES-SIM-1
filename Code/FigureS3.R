...
data_1900 <- (terra::rast(hist_path, 'secdf')[[1051]] + terra::rast(hist_path, 'secdn')[[1051]])*100  #app(terra::rast(hist_path, 'secdn'), mean, na.rm=T) + app(terra::rast(hist_path, 'secdn'), mean, na.rm=T)
data_2015 <- (terra::rast(hist_path, 'secdf')[[1166]] + terra::rast(hist_path, 'secdn')[[1166]])*100  #app(terra::rast(hist_path, 'secdn'), mean, na.rm=T) + app(terra::rast(hist_path, 'secdn'), mean, na.rm=T)
data_2015_ssp1 <- (terra::rast(ssp1_path, 'secdf')[[1]] + terra::rast(ssp1_path, 'secdn')[[1]])*100
data_2050_ssp1 <- (terra::rast(ssp1_path, 'secdf')[[36]] + terra::rast(ssp1_path, 'secdn')[[36]])*100
data_2015_ssp3 <- (terra::rast(ssp3_path, 'secdf')[[1]] + terra::rast(ssp3_path, 'secdn')[[1]])*100
data_2050_ssp3 <- (terra::rast(ssp3_path, 'secdf')[[36]] + terra::rast(ssp3_path, 'secdn')[[36]])*100
data_2015_ssp5 <- (terra::rast(ssp5_path, 'secdf')[[1]] + terra::rast(ssp5_path, 'secdn')[[1]])*100
data_2050_ssp5 <- (terra::rast(ssp5_path, 'secdf')[[36]] + terra::rast(ssp5_path, 'secdn')[[36]])*100
...
q <- unique(q)
br <- c(q[1], q[4], q[6], q[8], q[10],q[13], q[15], q[18])
br_at <- c(0.5, 3.5, 5.5, 7.5, 9.5, 12.5, 14.5, 17.5)
...
dev.copy(pdf,file.path(root_figures, 'FigureS3_legend_change.pdf'),
         width=20,height=2.9)
dev.off()
...
all <- na.omit(all)
q <- classIntervals(all$secdf_1051, 20, style="quantile")$brks 
q <- unique(q)
br <- c(q[1], q[4], q[6], q[8], q[10], q[12], q[15])
br_at <- c(0.5, 3.5, 5.5, 7.5, 9.5, 11.5, 14.5)
...
#save as file
dev.copy(pdf,file.path(root_figures, 'FigureS3_SecondaryLand.pdf'),
         width=16,height=14)
dev.off()
...
dev.copy(pdf,file.path(root_figures, 'FigureS3_legend_1900.pdf'),
         width=20,height=3)
dev.off()
