...
# write_csv2(all_data1,paste0(root,"/Figures/Figure4/alldata1.csv"))
# write.csv(final_data,paste0(root,"/Figures/Figure4/BD_ES_regional_data_paper9.csv")
...
map_simp.kt<- sf::st_read(file.path(root_IPBES_Regions,"IPBES_subregions_simp_v5.shp"))
...
# 
# write.csv2(map.test.centroids1,"../henrique/Dropbox (iDiv)/IPBES_BES-SIM/Publications/Synthesis/Science_Sub2/Figures/Figure4/mapcentroids.txt")

# the point fo Europe was out of place and I moved it 30 degrees to the left (now 10,5327451358187, before 40,5327451358187)

map.test.centroids1<-read.csv2("Figure4_mapcentroids.txt")
...
for (i in 1:3){
  ...
  bar_annotation_list <- lapply(1:length(map.kt$OBJECTID), function(j) 
